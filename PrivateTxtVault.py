...
def verify_sha256():
    ...
    age_path = normalize_user_path(raw_path)
...
def encrypt_file():
    """Encrypt a .txt file using age.exe passphrase mode and verify the result."""
    ensure_directories()
    ...
    txt_path = normalize_user_path(raw_path)
...
def decrypt_file():
    """Decrypt a .age file using age.exe and remove partial output on failure."""
    ensure_directories()
    age_exe = find_age_exe()
    if age_exe is None:
        print("ERROR: age.exe was not found next to PrivateTxtVault.py.")
...
def main():
    if len(sys.argv) > 2:
        print("Usage: python PrivateTxtVault.py [--audit]")
        return 2
    if len(sys.argv) == 2:
        if sys.argv[1] == "--audit":
